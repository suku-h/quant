...
def analyze_indicator(file, max_buy_val, min_sell_val, aroon_period, adx_period, min_adx):
    ...
    for count, ticker in enumerate(stocks):
        ...
        df['Chaikin'] = talib.ADOSC(df['High'].values, df['Low'].values, df['Close'].values, df['Volume'].values, 3, 10)
        ...
        if ticker == 'RELIANCE':
            df.to_csv('sl - Copy.csv')

        # don't check volume as volume already part of the calculation
        for i in range(len(op) - 1):
            if op[i] > buy_signal and adx[i] > min_adx and i > closePosition:
                addTrade(trades, ticker, dates[i + 1][1], vals[i + 1], volume=vol[i+1], indicatorVal = indicatorVals[i])
                noBuys = False
                for j in range(i + 1, len(op) - 1):
                    if op[j] < sell_signal and indicatorVals[j] <= indicatorVals[j - 1]:
                        addTrade(trades, ticker, dates[j + 1][1], vals[j + 1], indicatorVal= indicatorVals[j], isBuy= False)
                        amt[j] = vals[j] - vals[i]
...
def analyseAROONOSC():
    ...
    minSellVals = np.array([50])
    periods = np.array([6])
    adx_periods = np.array([6])
    adx_mins = np.array([30])
